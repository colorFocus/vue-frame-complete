...
(function () {
    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                actionUrl = editor.getOpt('imageAction'),
                isNeedRewriteImageUri = editor.getOpt('isNeedRewriteImageUri'),//是否需要更换地址
                reWriteImageUriTemplate = editor.getOpt('reWriteImageUriTemplate'),//得到重写模板
                reWriteImageIdx = editor.getOpt('reWriteImageIdx'),//替换字符串
            ...
            if (!WebUploader.Uploader.support()) {
                ...
                return;
            } else if (!editor.getOpt('imageAction')) {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.code == '0') {
                        if(isNeedRewriteImageUri===true){
                            var resData = json.data;
                            if(resData && resData.length > 0){
                                var id = resData[0].fileId;
                                if(id && id.length > 0){
                                    var imgUri = reWriteImageUriTemplate.replace(reWriteImageIdx,id);
