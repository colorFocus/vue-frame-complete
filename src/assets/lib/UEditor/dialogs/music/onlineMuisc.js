    ...
    OnlineMuisc.prototype = {
        ...
        _sent:function (param) {
            ...
            utils.loadFile(document, {
                src:me.dataUrl + '&query=' + param + '&page_size=' + me.total + '&callback=onlineMuisc.callback&.r=' + Math.random(),
                tag:"script",
        ...
        _renderTemplate:function (data) {
            ...
            for (var i = 0, tmpList; tmpList = data[i++];) {
                ...
                if (i == 1) {
                    ...
                    if (data.length != 1) {
                        t.push('<div id="page' + i + '" onclick="onlineMuisc.onpageclick(' + i + ')" class="pageon">' + (i ) + '</div>');
                    }
                } else {
                    s.push('<div id="panel' + i + '" class="paneloff">');
                    t.push('<div id="page' + i + '" onclick="onlineMuisc.onpageclick(' + i + ')" class="pageoff">' + (i ) + '</div>');
                }
                ...
                for (var j = 0, tmpObj; tmpObj = tmpList[j++];) {
                    ...
                    s.push('<input type="radio" id="radio-' + i + '-' + j + '" name="musicId" class="m-l" onclick="onlineMuisc.doselect(' + (me.pageSize * (i-1) + (j-1)) + ')"/>');
                    ...
                    s.push('<span class="m-try" onclick="onlineMuisc.doselect(' + (me.pageSize * (i-1) + (j-1)) + ');onlineMuisc.listenTest(this)"></span>');
                    s.push('</label>');
                }
                s.push('</div>');
                s.push('</div>');
