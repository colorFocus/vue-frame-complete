...
(function(){
    ...
    UploadFile.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                uploader,
                actionUrl = editor.getOpt('videoAction'),
                isNeedRewriteVideoUri = editor.getOpt('isNeedRewriteVideoUri'),//是否需要更换地址
                reWriteVideoUriTemplate = editor.getOpt('reWriteVideoUriTemplate'),//得到重写模板
                reWriteVideoIdx = editor.getOpt('reWriteVideoIdx'),//替换字符串
                videoMaxSize = editor.getOpt('videoMaxSize'),
                acceptExtensions = (editor.getOpt('videoAllowFiles') || []).join(','),
                videoResId = editor.getOpt('videoResId');
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.code == '0') {
                        if(isNeedRewriteVideoUri===true){
                            let id = json['data'][videoResId];
                            if(id && id.length > 0){
                                let imgUri = reWriteVideoUriTemplate.replace(reWriteVideoIdx,id);
                                json.data.url = imgUri;
                            }
                        }
